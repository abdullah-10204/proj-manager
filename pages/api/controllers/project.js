...
export const getProjects = async (req, res) => {
  try {
    ...
    if (role !== "Admin") {
      ...
      adminEmailToMatch = user.createdBy;
    }
...
export const addFolder = async (req, res) => {
  try {
    ...
    if (!name || !email) {
      return res.status(400).json({ error: "Folder name and email are required" });
    }
...
export const uploadFileToSubfolder = async (req, res) => {
  try {
    ...
    const { projectId, folderId, subfolderId, fileUrl, fileName, email } = req.body;

    if (!fileUrl || !fileName || !email) {
      return res.status(400).json({ message: 'File URL, name, and email are required' });
    }
...
export const updateFile = async (req, res) => {
  const { projectId, fileId, newName, folderId, isNested, parentFolderId, email } = req.body;
...
export const deleteFile = async (req, res) => {
  const { projectId, fileId, folderId, isNested, parentFolderId, email } = req.body;
